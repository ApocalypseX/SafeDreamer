...
def train_eval(
  ...
  while len(train_replay) < max(args.batch_steps, args.train_fill):
    driver_train(random_agent.policy, steps=100, lag=lag.lagrange_penalty)
  print('Prefill eval dataset.')
  while len(eval_replay) < max(args.batch_steps, args.eval_fill):
    driver_eval(random_agent.policy, steps=100, lag=lag.lagrange_penalty)
  ...
  while step < args.steps:
    if should_eval(step):
      ...
      driver_eval.reset()
      driver_eval(policy_eval, episodes=max(len(eval_env), args.eval_eps), lag=lag.lagrange_penalty)
    driver_train(policy_train, steps=100, lag=lag.lagrange_penalty)
    if should_save(step):
      checkpoint.save()
  logger.write()
  logger.write()
